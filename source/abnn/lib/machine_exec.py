...
def run_node_tasks(max_thread,
    ...
    task_chunks = [all_task[i:i + numb_jobs]
                   for i in range(0, len(all_task), numb_jobs)]
    base_path = os.getcwd() + "/"
    count_batch = 0
...
def exec_hosts(machine_env,
    ...
    task_chunks = [task_dirs[i:i + numb_jobs]
                   for i in range(0, ntasks, numb_jobs)]
    args_chunks = [task_args[i:i + numb_jobs]
                   for i in range(0, ntasks, numb_jobs)]
    ...
    for ii in range(nbatch):
        ...
        for jj in range(len(task_batch)):
            ...
            work_name = os.path.basename(work_path)
            logging.info(("%s %03d %s: %s %s" %
                         (host, ii, work_name, cmd, work_args)))
...
def exec_hosts_batch(machine_env,
    ...
    for ii in range(nbatch):
        task_batch = task_chunks[ii]
        args_batch = args_chunks[ii]
        logging.info(("%s %03d : %s with %d jobs %s" %
                     (host_list, ii, cmd, len(task_batch), task_batch)))
        ps = machine_env.exec_batch(
            cmd, ".", task_batch, task_args, work_thread)
        while True:
            if not(any(p.wait() for p in ps)):
                break
            time.sleep(1)
