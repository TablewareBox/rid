...
def exec_batch(cmd,
    ...
    for ii, mydir in enumerate(task_dirs):
        ...
        with open('sub', 'w') as fp:
            fp.write(make_slurm_script(cmd, work_thread, numb_gpu,
                     myarg, time_limit, mem_limit, modules, sources, fin_tag))
...
def exec_batch_group(cmd,
    ...
    task_chunks = [task_dirs[i:i + group_size]
                   for i in range(0, ntasks, group_size)]
    args_chunks = [task_args[i:i + group_size]
                   for i in range(0, ntasks, group_size)]
    ...
    for ii in range(len(task_chunks)):
        ...
        os.chdir(group_dir)
        with open('sub', 'w') as fp:
            fp.write(make_slurm_script_group(
                cmd, task_chunks[ii], work_thread, numb_gpu, args_chunks[ii], time_limit, mem_limit, modules, sources, fin_tag))
            job = SlurmJob(os.getcwd(), 'sub', job_finish_tag=fin_tag)
        job_list.append(job)
        os.chdir(working_dir)
    os.chdir(cwd)
