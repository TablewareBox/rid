    ...
    for walker_idx in range(numb_walkers):
        ...
        if (iter_index > 0):
            prev_temp_path = make_iter_name(
                iter_index-1) + "/" + temp_name + "/" + make_walker_name(walker_idx) + "/"
...
def make_next_temp(graph_list,
    ...
    for ii in range(0, nframes-1, temp_test_intvl):
        ...
        d = make_temp_decision(sample_dev, dev_trust_lvl,
                               temp_incr_lvl, temp_decr_lvl)
...
def run_iter(json_file, init_model):
    ...
    for ii in range(numb_iter):
        if ii > 0:
            prev_model = glob.glob(make_iter_name(
                ii-1) + "/" + train_name + "/*pb")
